...
exports.createFood = async (request, response) => {
    try {
        ...
        response.status(201).send({ food: newFood });
    } catch (error) {
        console.log(error);
...
exports.readOneFood = async (request, response) => {
    try {
        const food = await Food.Food.findByPk(request.params.id);
...
exports.updateFood = async (request, response) => {
    try {
        ...
        await Food.Food.update({ name: request.body.name.toLowerCase() }, { where: { id: request.params.id } });
        const food = await Food.Food.findByPk(request.params.id);
        response.status(200).send({ food: food });
    } catch (error) {
        console.log(error);
